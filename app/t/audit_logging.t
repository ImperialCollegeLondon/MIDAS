...
my $tmp_dir = dir('/tmp' ,'__midas_audit_logs');
$tmp_dir->rmtree;
$tmp_dir->mkpath;

my $log_file = file $tmp_dir, 'midas.log';
...
my @log_file_contents = $log_file->slurp;
...
my @log_files = sort { $a cmp $b } dir($tmp_dir)->children;
is scalar @log_files, 2, 'found active and rotated logs';

@log_file_contents = $log_files[0]->slurp;
is scalar @log_file_contents, 5, 'found 5 rows in active log';

@log_file_contents = $log_files[1]->slurp;
